...
public class CategoryController {
    ...
    @PutMapping(value = "/{id}")
    public ResponseEntity<ResultDto> editCategoryById(@Valid @PathVariable Long id, @RequestBody CategoryRequest categoryRequest) throws CustomException {
        ResultDto resultDto = categoryService.edit(id, categoryRequest);
        return new ResponseEntity<>(resultDto, HttpStatus.OK);
    }

    @GetMapping(value = "/{id}")
    public ResponseEntity<Category> getCategoryById(@PathVariable Long id) throws CustomException {
        Category category = categoryService.findCategoryById(id);
        return new ResponseEntity<>(category, HttpStatus.OK);
    }

    @DeleteMapping(value = "/{id}")
    public ResponseEntity<ResultDto> deleteCategoryById(@PathVariable Long id) throws CustomException {
        ResultDto resultDto = categoryService.deleteCategoryById(id);
        return new ResponseEntity<>(resultDto, HttpStatus.OK);
    }

    @PostMapping(value = "/search")
    public ResponseEntity<Datatable> getCategoryPage(@RequestBody CategorySearchRequest categorySearchRequest) {
